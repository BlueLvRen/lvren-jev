...
**将业务判断写进配置，将结构化结果直接交给业务代码。接入下一个场景时，复用已有的调用和处理方式。**

以工时分类为例：调用 Jev 得到一个答案之后，应用还需要知道 `operations` 对应“运维”、置信度多低应转为“待确认”，以及怎样将结果交给后续流程。当工单分派、邮件分类也需要这些能力时，请求构造、结果解析和分类策略就会成为反复编写的代码。

`lvren-jev` 将这些工作整理为可复用的配置和接口：

| 接入业务时需要解决的问题 | lvren-jev 提供什么 |
| --- | --- |
| 类别、评分标准和判断条件随业务调整 | 用 YAML/JSON 定义决策，加载时校验；修改定义并重新加载即可复用原有调用流程 |
| 模型答案需要转换为业务可用的数据 | 返回 `ClassificationResult`、`ScoreResult` 或 `NoulResult`，通过明确的属性读取结果，也可转换为字典 |
| 分类结果不够确定时怎样处理 | 按配置的置信度阈值返回兜底类别，保留概率及 `fallback` 标记，供业务安排复核 |
| 多个决策需要一致的调用配置 | 共用 `JevRuntime`，统一配置连接、超时、重试和可选的进程内缓存 |
| 从单个判断扩展到多个判断 | 分类、评分、条件判断使用同一套运行时，也可通过 `DecisionRequest` 在一次请求中组合多个问题 |

因此，接入第二个分类场景时，通常只需换一份决策定义、接上新的数据读写，便可继续使用 `classify()` 和同样的结果结构。业务代码可以集中在“拿到数据后做什么、得到结果后做什么”。

本包底层仍通过官方 `typesafe-sdk` 调用 Jev。若只想试验一次原始请求，直接使用官方 SDK 很合适；当应用需要持续维护决策定义、统一结果处理，或让多个业务复用接入方式时，`lvren-jev` 的价值在于减少重复接入代码，让业务变化主要落在决策定义和业务流程中。
