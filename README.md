...
以完整业务操作作为 LLM 的委派边界，将决策细节和执行过程封装在程序内部。

## 为什么使用 lvren-jev

官方 Jev SDK 解决“如何调用 Jev API”；`lvren-jev` 将 Jev 的分类、评分和条件判断封装为可复用的决策原语，供普通程序构建业务能力，再交由 LLM 按需调用。

直接让 LLM 操作 SDK，仍需它理解参数、组织判断指令并解释原始响应，可能增加上下文和协调成本。`lvren-jev` 将这些细节固定在决策定义与程序接口中，让三者各司其职：

| 角色 | 职责 |
| --- | --- |
| LLM | 理解任务、选择业务操作，根据完成状态继续后续任务 |
| 普通程序 | 获取语义上下文、执行决策、处理结果并完成业务操作，回报状态及必要产物 |
| lvren-jev | 加载决策定义、调用 Jev，返回结构化的分类、评分或条件概率结果 |
