...
`lvren-jev` 将 TypeSafe Jev 的分类、评分和条件判断封装为应用与 LLM 可以复用的决策原语：预先定义一个判断的输入、标准和结果，之后像调用普通函数一样使用它。业务程序可以直接调用 Python 接口；LLM 所在的应用可以将已配置的决策暴露为工具，供 LLM 传入业务数据并使用结构化结果。
...
`lvren-jev` 的核心思路是：**把决策定义一次，让调用方反复使用。** 类别、判断指令、评分标准和分类回退策略保存在决策定义中，由库加载、校验并构造 Jev 请求。使用已经配置好的决策时，LLM 无需每次重新编写提示词、拼装底层参数或解析原始响应，也无需为分类、评分和条件判断各设计一套接入方式。

例如，应用预先加载“工时分类”定义，并将分类器暴露为 `classify_worklog(description)` 工具。LLM 只需传入工时内容，便能拿到类别、置信度和是否需要复核的结构化结果。它需要理解的是这个业务工具的用途和输入输出，而不是 Jev 的请求协议。工具注册由宿主应用负责，决策执行由 `lvren-jev` 完成。

这种复用建立在四个约定上：

- **统一的决策定义**：用 YAML/JSON 描述分类、评分或条件判断，定义与运行配置、业务代码彼此分离。
- **简单的调用入口**：单个决策使用 `classify()` 或 `evaluate()`，业务程序可以直接调用，也可由宿主应用包装成 LLM 工具。
- **结构化的结果**：返回分类、评分或条件概率对象，可通过 `to_dict()` 转为字典，供程序或工具调用方继续处理。
...
它的目标是让现有 LLM 和业务系统以低耦合、结构化、可复用的方式使用 Jev，减少每次调用时重新理解协议和组织决策的工作。具体节省多少 token、延迟或费用，需要结合实际任务衡量。
